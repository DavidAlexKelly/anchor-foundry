...
- **Testing standard is unchanged**: real Postgres, real OpenSearch, real dev servers, real browser. Two of the four defects found this week were invisible to the API tests and obvious in a browser (`STATUS.md` §52) — Workshop is far more interactive than anything built so far, and Playwright coverage of the builder is not optional. **Now in the repo** (`STATUS.md` §107): `e2e/` runs against the real stack, `scripts/check.sh` runs everything, and `.github/workflows/ci.yml` runs that on a PR. The Pivot Table and Time Series are covered; the chart drill-down, Card List and Search checks are still scratchpad scripts and should move next. **Still missing**: a JavaScript test runner, so the pure functions in the widget layer have no unit tests.
