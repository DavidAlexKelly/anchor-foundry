...
### 2.5 Transforms authoring — **L, the path is built end to end** (`docs/decisions/0004-running-customer-code.md`; `STATUS.md` §63 declarations, §64 the runner task and empty role, §65 the container entrypoint, §66 the EFS scratch transport, §67 dispatch). **What remains is the substitution**: `jobs/model_runs.py` still runs Python transforms through `python_sandbox.py`'s subprocess, which §63 was explicit is process isolation and not a security boundary. Pointing that call site at `transform_dispatch.run_transform` is what actually moves customer code into the isolated runner — small in diff, and it changes where every existing Python transform runs, so it wants its own verification rather than being a rider on the last item.
