...
_Phase 1 built six pillars and is complete: it lives in `docs/roadmap-phase-1-pillars.md`, and `STATUS.md` §21–§65 is the detail. **Cross-references elsewhere in the repo of the form "ROADMAP.md section N item M" mean that document, not this one.**_
...
| 3 | ~~**Pivot Table**~~ | **Done** (`STATUS.md` §105). Counts by two properties at once, over an object set. **The axes are grouped counts**, built by the same `group_object_set` a chart plots, so a row total and a bar cannot disagree — which forces the honest consequence into the open: a row's cells sum to *less* than its total wherever an object has no value for the column property or a column was capped, and the widget says so rather than making the margins add up. Clicking a cell writes two equality clauses, the chart drill-down's mechanism with one more clause; a cross-tab of a property against itself is refused, because a diagonal is a grouped count in a grid's clothes |
...
| 3 | ~~**Time Series / Timeline**~~ | **Done** (`STATUS.md` §106), over `updated_at` — **and the widget says that is what it plots**. `updated_at` is a real `timestamptz` on one store and a mapped `date` on the other, so both bucket it identically by the calendar in UTC; a resync moves every object to today, so this answers "what has been changing" and not "when did things happen", and a line of counts reads as the second unless it says otherwise. Bucketing a *date property* is the blocked half — untyped properties, decision 0006, the same wall ordered operators hit. Empty buckets are filled in one place so the two stores cannot fill differently, the range is the data's own first and last rather than "the last 30 days", and too long a span **refuses and names a coarser interval** — a truncated series is a different period, not a shorter one. No drill-down: a time bucket is a range over a system field, which is not in the property-equality vocabulary every narrowing widget shares |
| 4 | ~~**Embedded module**~~ | **Done** (`STATUS.md` §114). One module inside another, rendered in a *nested* Craft.js editor that is always disabled — editing a module means opening it, and two editable canvases on one screen have no way to say which document a gesture meant. **The inner module resolves its own variables and shares none with its host**: a shared namespace would collide the first time two modules both declared `v_filter`, and the collision would be silent. What may be embedded is settled when the author saves (`routes/canvas.py`) — no self-embedding, no cycles, nothing outside the project, nothing deeper than three — because a cycle found at render time is a browser that hangs for a viewer who cannot fix it |
| 4 | **Comments / Notepad** | |
...
- **Testing standard is unchanged**: real Postgres, real OpenSearch, real dev servers, real browser. Two of the four defects found this week were invisible to the API tests and obvious in a browser (`STATUS.md` §52) — Workshop is far more interactive than anything built so far, and Playwright coverage of the builder is not optional. **Now in the repo** (`STATUS.md` §107): `e2e/` runs against the real stack, `scripts/check.sh` runs everything, and `.github/workflows/ci.yml` runs that on a PR. The Pivot Table, Time Series, chart drill-down, Card List and Search are covered (§107, §108); `e2e/README.md` names what is not. Vitest covers the widget layer's pure functions (`STATUS.md` §111) — arithmetic and formatting only, with the component boundary written down, since a jsdom test that passes while the real thing is broken is the defect this standard exists to prevent.
