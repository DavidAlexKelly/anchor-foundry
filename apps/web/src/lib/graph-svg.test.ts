...
import { NODE_W, nodeX, nodeY } from "./pipeline-graph";

const node = (
  id: string, layer: number, position: number, over: Partial<DrawableNode> = {},
): DrawableNode => ({ id, kind: "dataset", name: id, layer, position, ...over });
...
describe("the picture", () => {
  ...
  it("puts a card where the screen puts it", () => {
    ...
    const svg = svgFor([node("mid", 2, 1)], []);
    expect(svg).toContain(`x="${nodeX(2)}" y="${nodeY(1)}"`);
  });

  it("draws one path per edge", () => {
    expect(svgFor(NODES, EDGES).match(/<path /g)).toHaveLength(2);
  });

  it("does not draw an edge to a node it is not drawing", () => {
    // A line reaching off the picture reads as a node the export lost.
    const svg = svgFor(NODES, [...EDGES, { from: "clean", to: "elsewhere" }]);
    expect(svg.match(/<path /g)).toHaveLength(2);
  });

  it("carries the colouring that was on screen", () => {
    const svg = svgFor(NODES, EDGES, { colours: { raw: "var(--line-strong)" } });
  ...
  it("names the two kinds whose word is not their column", () => {
    const svg = svgFor(
      [node("o", 0, 0, { kind: "object_type" }), node("c", 0, 1, { kind: "connection" })],
      [],
    );
  ...
  it("is wide enough for the layers it drew", () => {
    const svg = svgFor([node("far", 6, 0)], []);
    const width = Number(svg.match(/ width="(\d+)"/)![1]);
    expect(width).toBeGreaterThanOrEqual(nodeX(6) + NODE_W);
  });

  it("carries a title, and escapes it too", () => {
    expect(svgFor([], [], { title: "A & B" })).toContain("<title>A &amp; B</title>");
  });
});
