...
async def resource_counts(conn: AsyncConnection, project_id: UUID) -> dict[str, int]:
    ...
    row = await fetch_one(
        ...
        SELECT
          ...
          -- **Repositories again (§291).** This counted `models` - the same
          -- rows Models counts - because decision 0001 made the Code pillar a
          -- view over model_versions, and the comment here said `code_repos`
          -- "has never had a row written to it". That stopped being true at
          -- §94, and §291 made the pillar the list of repositories, so the
          -- badge and the page it labels had come apart: a project with one
          -- repository and forty transforms showed 40 beside a list of one.
