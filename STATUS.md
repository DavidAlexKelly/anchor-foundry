...
**And one of the twenty-four escaped into the branch, which is the finding worth
more than the feature.** The harness restores from `git show HEAD:<path>`; I
committed while it was running, HEAD moved to a tree holding the applied mutant,
and the restore faithfully wrote *that* back. `git status` was clean, because the
mutation had been committed. Every signal agreed the tree was pristine, and the
descending `<option>` stayed missing from `PropertySortField` until the full
browser suite went red 45 minutes later on three panel tests that had passed
per-file an hour before. §220's rule one turn out: a restore narrower than its
mutants is worse than no harness, and a restore reading from a **moving
baseline** is not narrower, it is wrong. The harness refuses to start over a
dirty tree; it cannot refuse a commit made underneath it.
