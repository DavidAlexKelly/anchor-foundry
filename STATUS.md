...
- **The residue is now large enough to be a clock, and it caught `test_ontology_search.py` in §185.** Every browser run's `Module.object_type()` creates an object type and nothing cleans up, by design - and this sandbox's shared database has reached **6,281 object types and 13,634 properties**. `GET /ontology-search` scans them, and one authenticated `curl` measures it at **4.5 seconds**, against a Playwright assertion whose default timeout is 5. So two of that file's tests fail on "Searching…" with nothing wrong in the diff, and they pass again on a fresh database. **Two separate things to take from it, and only one is a test problem.** The test problem is the 5-second default: those assertions should carry an explicit timeout, as the deep-link and versions checks already do. The other is not - a workspace with six thousand object types is not absurd for a real deployment, and an ontology search that takes four and a half seconds there is a product finding this sandbox surfaced for free. Neither is in §185's scope; both are written down here rather than fixed in passing.
