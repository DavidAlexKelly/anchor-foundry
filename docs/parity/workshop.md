...
Ours in the right-hand column. **24 of ~52 rows below are ✅ or ◑** — counted from the table rather than carried forward, because the number written here had drifted six behind it by §212 and a tally nobody recomputes is worse than no tally.
...
| Object Selector — select many objects | ○ — p.444's one line is its whole specification; it has no page of its own. §214's Object Dropdown is its single-selection half, and the pieces the multiple one needs — `toggle`, a clause list of several keys — are already in `object-table-selection.ts` |
...
6. **Inline Action depth**, ~~Object Dropdown~~, Object Selector — **Object Dropdown is done (§214)**, and it needed nothing new: p.457's output is the Object Table's selection clauses and p.457's Allow no selection is p.224's auto-selection inverted. **The Object Selector is now the cheap one** and should be taken next, before Inline Action depth: p.444 gives it one line and no page, its multiple-selection pieces are already in `object-table-selection.ts`, and every other setting is the Dropdown's. What §214 *did* turn up is a real dependency for both: p.458's per-property sort is refused by `object_sets.parse_sort` (decision 0006), so it waits on declared property types along with the Object Table's p.223 default sorts — those two are one unit, not two
