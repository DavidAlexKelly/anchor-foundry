...
5. **Object View widget, Property List, Links, Object Set Title** — **Object Set Title (§210) and Property List (§211) are done**, and neither depended on ontology work: both needed only the object set variables, title properties and property renderer that already existed. **The premise of this item was wrong twice**, which is worth saying before the third: what actually blocks the remaining two is narrower than "ontology work". **Links** wants link traversal reaching a widget — `traverse_set` exists (§155), so this is closer than it reads. The **Object View widget** genuinely does want `ontology.md`'s Object Views, because it renders one
