...
5. **Object View widget, Property List, Links, Object Set Title** — **Object Set Title (§210), Property List (§211) and Links (§212) are done**, and none of them depended on ontology work: they needed the object set variables, title properties, property renderer and link traversal that already existed. **The premise of this item was wrong three times.** It was written down as wrong twice before Links, and Links then confirmed it a third time from a *different* direction: the traversal was there, but the widget still needed a new endpoint — `GET /object-types/{id}/links` — because a builder configures a widget before there is data to traverse, and no amount of existing traversal answers a question asked with no object in hand. The lesson to carry forward is that "does the data layer support this" and "can this be *configured*" are separate questions. The **Object View widget** is the one that genuinely does want `ontology.md`'s Object Views, because it renders one; the exploration, open-view and hover-preview affordances left ○ on the Links row are waiting on the same thing
