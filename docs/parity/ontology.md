...
7. ~~**Struct property type**~~, ~~then the Ontology Manager editor~~, then **Workshop struct variables**. — **the type is built (§245) and so is the editor (§246)**; the Workshop variable is not. Struck in thirds rather than whole, because §216's rule is that a line goes stale in the commit that finishes part of it. What is left is `workshop.md` §3.2's struct variable together with p.143's **Extract struct field** transform, and they are one item rather than two: p.155 says "widgets and variable transformation operations cannot use structs as a whole, so individual struct fields must be extracted for use", which makes the variable close to useless without the transform. **Reading the pages first changed the shape of this line** — it reads as one item and it is at least three, which is the direction every build-order estimate in this repo has been wrong in (§232, §235).
