...
7. **Struct property type**, then Workshop struct variables. — **the type is built (§245)**; the editor and the Workshop variable are not. Struck in halves rather than whole, because §216's rule is that a line goes stale in the commit that finishes part of it: what is left is p.152–160's Ontology Manager surface (a struct is declarable through the API only) and `workshop.md` §3.2's struct variable with p.143's **Extract struct field** transform, which p.155 makes the *only* way a widget can use one ("widgets and variable transformation operations cannot use structs as a whole"). **Reading the pages first changed the shape of this line**: it reads as one item and it is at least three, which is the direction every build-order estimate in this repo has been wrong in (§232, §235).
