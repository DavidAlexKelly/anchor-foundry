...
| **Derived properties** — calculated at runtime from other properties or links | ◑ | §161 — p.143-148, the **declaration only**. A chain of up to three link types (p.147), an aggregation (p.145) and the property at the far end (p.146) can be declared, validated against the workspace's link types, and stored. **Nothing evaluates one yet**, so it is not exposed in the property editor — a settable property that renders nothing would be worse than an absent one. Refusals are built: a chain that does not join up, a link with no join, a missing aggregation where a hop reaches many, `count` with a property, a limit on an aggregation that collects nothing, and p.148's own list (not required, not formatted, and — ours — not also edit-only). **Four of p.145's nine aggregations are refused rather than answered**: `sum`/`avg`/`min`/`max` need to know a property is a number, which is the untyped-index blocker §52, §74, §83 and §86 already wait on — this is the fifth — and `approx_cardinality` because OpenSearch approximates where Postgres is exact. p.143's own first example, a department's average employee salary, is among the refused |
