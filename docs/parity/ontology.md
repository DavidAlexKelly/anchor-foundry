...
| **Derived properties** — calculated at runtime from other properties or links | ◑ | §161 declares one, §162 **answers** it. A chain of up to three link types (p.147), an aggregation (p.145) and the property at the far end (p.146), evaluated when an object is read and never stored (p.143's "instead of storing data directly"). The chain is an object set rooted at the one object, so §155's traversal answers it and nothing new was needed. **Two limits, both named**: evaluated on the *single-object* read only — a page of a table would be an N+1 per row, which needs the aggregation pushed into the index — and **four of p.145's nine aggregations are refused**, `sum`/`avg`/`min`/`max` on the untyped-index blocker (§52, §74, §83, §86; this is the fifth) and `approx_cardinality` because OpenSearch approximates where Postgres is exact. p.143's own first example, a department's average employee salary, is among the refused. Not exposed in the property editor yet, so a derivation is set through the API |
