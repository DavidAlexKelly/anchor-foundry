...
| **Exports** — push data out | TOC §24–25 | ◑ §265, §267, §270 (decisions 0014, 0016) — **file exports** (S3) and **table exports** (Postgres, MySQL), p.192's nothing-to-export success, p.197's refusals, p.202's per-source switch, p.206's history, and p.205's **schedule** (§270 — a cron on the export, fired by the worker, with p.202's switch re-read every time it comes due). ○ for **streaming exports** (no streams to reverse), **export tasks** (p.211 recommends against them and p.206 says table exports replace them), **exportable markings** (p.202 — no marking system to draw a list from), and **four of p.195–196's six table modes**, which are defined over a transaction log `dataset_versions` does not keep |
...
3. ~~**Exports** — the reverse direction; currently data only flows in.~~ **Done (`STATUS.md` §265, §267)**: the server half, then the screen p.203 puts on the source itself. **Finished by §270**, which built p.205's schedule — the last piece this row named. Struck in thirds per §216. Decision 0014 records the design, and the part worth reading is **§2**: p.195–196 gives six table export modes and four of them are defined over `SNAPSHOT`/`APPEND`/`UPDATE`/`DELETE` transactions. `dataset_versions` is a "snapshot per sync/upload" — every version is a complete view and there is no transaction type on it — so those four could not do what their own names say. Two are built: `mirror` (p.195's *Full dataset with truncation*) and `full` (*Full dataset without truncation*, whose duplicates p.195 calls a feature). **The gap is in the dataset model, not in exports**, and closing it would change every writer in the platform. Also absent by decision: a REST source is not an export destination, because p.17 already lists webhooks as the way to write to one and §259–§262 built that — refused with a sentence pointing there rather than left out of a dropdown. §270's third is the schedule, and decision 0016 §1 is the part worth reading: **p.205's scheduling is not an export feature in Foundry at all** — it says to "configure as you would for any other job", meaning the platform's build scheduler reached through Data Lineage. This platform has no general scheduler, so an export cron is the third instance of a shape 0014 established rather than the first use of a general one. What that costs is a schedule that fires on the clock where Foundry's can fire on an upstream event, and what makes the gap small is a decision already made: p.192's skip means a poll with nothing new writes nothing and says so, so an hourly cron against a daily dataset is twenty-three cheap no-ops and one export.
