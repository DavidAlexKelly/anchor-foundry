...
| **Exports** — push data out | TOC §24–25 | ◑ §265 (decision 0014) — **file exports** (S3) and **table exports** (Postgres, MySQL), p.192's nothing-to-export success, p.197's refusals, p.202's per-source switch, and p.206's history. ○ for **streaming exports** (no streams to reverse), **export tasks** (p.211 recommends against them and p.206 says table exports replace them), **scheduling** (p.205 — the runner exists, the trigger does not, §267), **exportable markings** (p.202 — no marking system to draw a list from), and **four of p.195–196's six table modes**, which are defined over a transaction log `dataset_versions` does not keep |
