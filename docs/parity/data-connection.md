...
| Source exploration — browse a source before syncing | TOC §17 | ◑ §2 (`discover`, the tree) and §268 (decision 0015) — **preview**, p.143's "sample of the selected table", for all four connectors, egress-guarded and capped. ○ for the **relationship graph** (p.143 says it "is not always available" in Foundry either, and it needs foreign keys `ColumnInfo` does not carry), the **file-import filter** (p.160-161 defines it over `SNAPSHOT`/`APPEND`/`UPDATE` transactions — the same log decision 0014 §2 found missing), and the browser half: free-text search over the tree, and a path from a previewed table to a configured sync |
