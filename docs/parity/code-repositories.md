...
| **SQL Scratchpad** | ◑ §305 | the reference syntax and the run are built; the **favourites** and **history** tabs are not, and neither is the panel. p.15's `` SELECT * FROM `branch_A`.`/path/to/dataset` `` is **translated, not adopted**: Foundry runs Spark, where a backtick quotes an identifier, and DuckDB has no backtick syntax at all — it fails at the parser — so every reference is rewritten to a double-quoted identifier and the sampled input is created under that name. The **branch qualifier is parsed and then refused**, which is the decision on this row: datasets here are versioned rather than branched (db 0025, and Global Branching is out of scope in `README.md`), and the worst answer to `` `branch_A`.`/orders` `` is to ignore the qualifier and return the current version — that returns a table, so nobody checks it. A path resolves to its last segment, because projects here are flat and p.15 writes paths |
...
8. ~~**Foundry Explorer equivalent**~~ **(done, §303–§304)**. SQL Scratchpad: the syntax and the run are **done (§305)**; the panel, history and favourites remain.
