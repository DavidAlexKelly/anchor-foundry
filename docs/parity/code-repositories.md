...
| Panel | Status | Notes |
|---|---|---|
| **Foundry Explorer** | ◑ §303 | browse files and folders; select a dataset and Open it. **Ours browses the platform, not the repository**, and the substitution is the point: the Files tab is already a file tree, so a second one would be a second answer to what is in this repository — what the editor cannot see is everything *outside* it, which is what a transform reads and writes. Datasets and object types, each under its own heading, with p.13's one verb on each row. **Outside the "is a file open" branch**, unlike the four panels above it: those answer questions about the current file and have no subject without one, while the moment this is most useful is *before* there is a file. Object types are workspace-level, so the listing has to reach past the project or that section is empty in every project forever. ◑ rather than ✅ because **insert a reference** is not built — that is §304 |
...
8. ~~**Foundry Explorer equivalent**~~ **(browsing done, §303; inserting a reference is §304)**, SQL Scratchpad history and favourites.
