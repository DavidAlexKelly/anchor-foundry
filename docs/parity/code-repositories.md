...
| **Expectations** — data expectations on the head branch | ◑ | (p.54) — we have quality gating (§11) and `dataset_expectations` (db 0020: rule type, column, config, severity), not surfaced on a proposal. **The genuine next unit in this section**, and the only one of the six remaining rows whose ingredients all exist. Note what p.54 asks for and what it does not: *view the data expectations on the head branch* — which rules would run — not their results, since results need p.52's head build and this file has already decided twice (§364, §365) not to require one. |
