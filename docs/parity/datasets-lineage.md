...
| **Properties and Histogram** | ◑ §353, §354 | (p.8) — the single-node half is the graph's detail bar and the multi-node half is Frequent Columns, now over a selection. ○: p.8's histogram is of property *values* ("common properties and their values alongside the number of appearances of each value") and this counts column **names**, which is p.54's own feature rather than p.8's; and p.8's **Update selection** — drilling from a highlight down to a selection — is the step after it. Not a side panel either: both halves sit on the graph, because this platform's graph is a page rather than an app with six regions. |
| **Manage Builds** | ○ | |
| **Manage Schedules** | ◑ | |
| **Related Artifacts** | ○ | |
